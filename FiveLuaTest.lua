...
CreateSquareToggle(miscLeft, "fake korblox", false, function(s)
    if _G.ToggleFakeKorblox then _G.ToggleFakeKorblox(s) end
end)
...
local function IsVisible(targetPart)
    ...
    if not camera or not targetPart or not targetPart:IsDescendantOf(workspace) then return false end
...
RunService.RenderStepped:Connect(function()
    ...
    if _G.AimBotEnabled and cachedTargetPart and cachedTargetPart:IsDescendantOf(workspace) then
        local camera = workspace.CurrentCamera
        if camera then
            local dir = (cachedTargetPart.Position - camera.CFrame.Position)
...
if hookmetamethod then
    ...
    oldNamecall = hookmetamethod(game, "__namecall", function(self, ...)
        ...
        if _G.SilentAimEnabled and not checkcaller() and cachedTargetPart and cachedTargetPart:IsDescendantOf(workspace) then
            local lowerMethod = method:lower()
            if lowerMethod == "raycast" then
                if typeof(args[1]) == "Vector3" then
                    local origin = args[1]
                    local dir = (cachedTargetPart.Position - origin)
                    if dir.Magnitude > 0.001 then
                        args[2] = dir.Unit * 10000
            ...
            elseif lowerMethod == "findpartonray" or lowerMethod == "findpartonraywithignorelist" or lowerMethod == "findpartonraywithwhitelist" then
                if typeof(args[1]) == "Ray" then
                    local oldRay = args[1]
                    local origin = oldRay.Origin
                    local dir = (cachedTargetPart.Position - origin)
                    if dir.Magnitude > 0.001 then
                        args[1] = Ray.new(origin, dir.Unit * 10000)
                        if setnamecallmethod then
                            setnamecallmethod(method)
                        end
                        return oldNamecall(self, unpackFunc(args))
    ...
    local localMouse = Players.LocalPlayer and Players.LocalPlayer:GetMouse()
    local oldIndex
    oldIndex = hookmetamethod(game, "__index", function(self, key)
        if _G.SilentAimEnabled and not checkcaller() and cachedTargetPart and cachedTargetPart:IsDescendantOf(workspace) then
            if localMouse and self == localMouse then
...
-- FAKE KORBLOX (FIXED ASSET IDS & CFRAME SPAWN)
_G.FakeKorbloxEnabled = false
local korbloxConn = nil

local function removeKorblox(char)
    if not char then return end
    
    local fakeLeg = char:FindFirstChild("GxiFakeKorbloxLeg")
    if fakeLeg then 
        fakeLeg:Destroy() 
    end

    local legNames = {"RightUpperLeg", "RightLowerLeg", "RightFoot", "Right Leg"}
...
local function applyKorblox(char)
    ...
    local targetPart = isR15 and (char:FindFirstChild("RightUpperLeg") or char:WaitForChild("RightUpperLeg", 2))
                            or (char:FindFirstChild("Right Leg") or char:WaitForChild("Right Leg", 2))
    ...
    fakePart.Name = "GxiFakeKorbloxLeg"
    fakePart.Size = Vector3.new(1, 2, 1)
    fakePart.CanCollide = false
    fakePart.Massless = true
    fakePart.CFrame = targetPart.CFrame

    local mesh = Instance.new("SpecialMesh")
    mesh.MeshType = Enum.MeshType.FileMesh
    mesh.MeshId = "rbxassetid://90294209"
    mesh.TextureId = "rbxassetid://90290425"
...
function _G.ToggleFakeKorblox(state)
    ...
    korbloxConn = Players.LocalPlayer.CharacterAdded:Connect(function(newChar)
        task.wait(0.5)
        if _G.FakeKorbloxEnabled then
