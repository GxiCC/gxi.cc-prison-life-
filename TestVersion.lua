...
local function CreateTab(name)
    ...
    -- Две колонки внутри каждой вкладки
...
-- Добавление подзаголовка секции (как "esp", "chams", "world" на скриншоте)
...
-- Квадратный переключатель (Квадратик слева)
...
-- Открытие / Закрытие по клику на Вотермарку
                ...
                if humanoid and humanoid.Health > 0 then
                    ...
                    if targetPart then
                        if not _G.WallCheckEnabled or IsVisible(targetPart) then
                            ...
                            if onScreen then
                                local distance = (Vector2.new(screenPos.X, screenPos.Y) - mouseLoc).Magnitude
...
RunService.RenderStepped:Connect(function()
    if _G.AimBotEnabled then
        local targetPlayer = GetClosestPlayerInFOV(_G.AimBotTargetPart)
        if targetPlayer and targetPlayer.Character then
            local targetPart = targetPlayer.Character:FindFirstChild(_G.AimBotTargetPart) or targetPlayer.Character:FindFirstChild("Head")
            local camera = workspace.CurrentCamera
...
if hookmetamethod then
    ...
    oldNamecall = hookmetamethod(game, "__namecall", function(self, ...)
        ...
        if _G.SilentAimEnabled and (method == "Raycast" or method == "FindPartOnRayWithIgnoreList" or method == "FindPartOnRay") then
            local targetPlayer = GetClosestPlayerInFOV(_G.SilentAimTargetPart)
            if targetPlayer and targetPlayer.Character then
                local targetPart = targetPlayer.Character:FindFirstChild(_G.SilentAimTargetPart) or targetPlayer.Character:FindFirstChild("Head")
                if targetPart then
                    local origin = args[1]
    ...
    oldIndex = hookmetamethod(game, "__index", function(self, key)
        if _G.SilentAimEnabled and not checkcaller() and self == Players.LocalPlayer:GetMouse() and tostring(key) == "Hit" then
            local targetPlayer = GetClosestPlayerInFOV(_G.SilentAimTargetPart)
            if targetPlayer and targetPlayer.Character then
                local targetPart = targetPlayer.Character:FindFirstChild(_G.SilentAimTargetPart) or targetPlayer.Character:FindFirstChild("Head")
                if targetPart then
...
--------------------------------------------------
-- VISUALS MODULE (CHAMS WITH TRANSPARENCY)
--------------------------------------------------
