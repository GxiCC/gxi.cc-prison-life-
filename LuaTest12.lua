...
-- Специфичная для Prison Life проверка команд
local function IsEnemy(player)
    ...
    -- Охрана воюет со всеми (Заключенные и Криминалы)
    if myTeam == "Guards" then
        return enemyTeam == "Inmates" or enemyTeam == "Criminals"
    end

    -- Заключенные и Криминалы союзники, их враги — Охрана
...
-- Вспомогательная функция поиска частей тела для R6 и R15
    ...
    oldNamecall = hookmetamethod(game, "__namecall", makeClosure(function(self, ...)
        ...
        elseif method == "FindPartOnRay" or method == "FindPartOnRayWithIgnoreList" or method == "FindPartOnRayWithWhitelist" then
            if typeof(args[1]) == "Ray" then
                ...
                if direction.Magnitude > 0.001 then 
                    args[1] = Ray.new(oldRay.Origin, direction.Unit * 10000) 
                    return oldNamecall(self, unpackFunc(args)) 
                end
            end
        end
...
local function removeKorblox(char)
    if not char then return end
    if char:FindFirstChild("GxiFakeKorbloxLeg") then 
        char.GxiFakeKorbloxLeg:Destroy() 
    end
...
local function applyKorblox(char)
    ...
    mesh.MeshId = "http://www.roblox.com/asset/?id=139148411"
    mesh.TextureId = "http://www.roblox.com/asset/?id=139148332"

    if isR15 then 
        mesh.Scale = Vector3.new(1.05, 1.05, 1.05)
        fakeLeg.CFrame = targetPart.CFrame * CFrame.new(0, -0.75, 0)
    else 
        mesh.Scale = Vector3.new(1.05, 1.05, 1.05)
        fakeLeg.CFrame = targetPart.CFrame
    end

    fakeLeg.Parent = char

    local weld = Instance.new("WeldConstraint", fakeLeg) 
...
function _G.ToggleFakeKorblox(state)
    ...
    korbloxConn = Players.LocalPlayer.CharacterAdded:Connect(function(nc) 
        ...
        if Config.FakeKorbloxEnabled then 
            task.wait(0.2)
            applyKorblox(nc) 
        end 
    end)
...
function _G.ToggleArrestAura(state)
    ...
    arrestAuraConn = RunService.Heartbeat:Connect(function()
        if os.clock() - lastArrest < 0.15 then return end
        ...
        for _, player in ipairs(Players:GetPlayers()) do
            if player ~= Players.LocalPlayer and IsEnemy(player) then
                local char = player.Character
                local hum = char and char:FindFirstChildOfClass("Humanoid")
                local torso = char and (char:FindFirstChild("Torso") or char:FindFirstChild("UpperTorso"))
                local hrp = char and char:FindFirstChild("HumanoidRootPart")

                if char and hum and hum.Health > 0 and torso and hrp then
        ...
        if closestTorso then
            lastArrest = os.clock()
            pcall(function()
                game:GetService("ReplicatedStorage").Remotes.InteractWithItem:InvokeServer(closestTorso)
            end)
        end
    end)
end