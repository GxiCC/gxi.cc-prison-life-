...
local function createHealthBar(player)
    ...
    background.Color = Color3.fromRGB(12, 12, 12)
    ...
    local connection = RunService.RenderStepped:Connect(function()
        ...
        if hrpVis then
            local fixedHeight = 45
            local fixedWidth = 3

            local barX = math.floor(hrpPos.X - 22)
            local barY = math.floor(hrpPos.Y - (fixedHeight / 2))

            local healthPercent = math.clamp(hum.Health / hum.MaxHealth, 0, 1)
            local fillHeight = math.floor(fixedHeight * healthPercent)
            local fillY = barY + (fixedHeight - fillHeight)

            local hpValue = healthPercent * 100
            local barColor = Color3.fromRGB(0, 255, 0)
            if hpValue < 25 then
                barColor = Color3.fromRGB(255, 30, 30)
            elseif hpValue < 50 then
                barColor = Color3.fromRGB(255, 230, 0)
            elseif hpValue < 75 then
                barColor = Color3.fromRGB(255, 140, 0)
            end

            outline.Size = Vector2.new(fixedWidth + 2, fixedHeight + 2)
            outline.Position = Vector2.new(barX - 1, barY - 1)
            outline.Visible = true

            background.Size = Vector2.new(fixedWidth, fixedHeight)
            background.Position = Vector2.new(barX, barY)
            background.Visible = true

            fill.Size = Vector2.new(fixedWidth, fillHeight)
...
Players.PlayerAdded:Connect(function(player)
    if _G.EspSkeletonEnabled then createSkeleton(player) end
    if _G.EspHealthEnabled then createHealthBar(player) end
end)
