...
CreateToggle(VisualsTab, "esp health", function(state)
    if _G.ToggleEspHealth then
        _G.ToggleEspHealth(state)
    end
end)
...
local function removeHealthBar(player)
    if healthCache[player] then
        pcall(function() healthCache[player].Outline:Remove() end)
        pcall(function() healthCache[player].Background:Remove() end)
        pcall(function() healthCache[player].Fill:Remove() end)
        if healthCache[player].Connection then
            healthCache[player].Connection:Disconnect()
        end
...
local function createHealthBar(player)
    ...
    local connection = RunService.RenderStepped:Connect(function()
        if not _G.EspHealthEnabled then
            removeHealthBar(player)
            return
        end
        ...
        local topPos, topVis = camera:WorldToViewportPoint(cf.Position + Vector3.new(0, size.Y / 2, 0))
        local botPos, botVis = camera:WorldToViewportPoint(cf.Position - Vector3.new(0, size.Y / 2, 0))
        ...
        if topVis and botVis then
            ...
            local barWidth = 2
            local barX = boxX - barWidth - 5

            local healthPercent = math.clamp(hum.Health / hum.MaxHealth, 0, 1)
            local fillHeight = boxHeight * healthPercent
            local fillY = boxY + (boxHeight - fillHeight)

            -- Определение цвета по уровню здоровья
            local hpValue = healthPercent * 100
            local barColor = Color3.fromRGB(0, 255, 0) -- >= 75%
            if hpValue < 25 then
                barColor = Color3.fromRGB(255, 0, 0) -- < 25%
            elseif hpValue < 50 then
                barColor = Color3.fromRGB(255, 255, 0) -- < 50%
            elseif hpValue < 75 then
                barColor = Color3.fromRGB(255, 165, 0) -- < 75%
            end

            -- Контур (Ультратонкая окантовка, ровные острые углы)
            outline.Size = Vector2.new(barWidth + 2, boxHeight + 2)
            outline.Position = Vector2.new(barX - 1, boxY - 1)
            outline.Visible = true

            -- Задний фон полосы
            background.Size = Vector2.new(barWidth, boxHeight)
            background.Position = Vector2.new(barX, boxY)
...
function _G.ToggleEspHealth(state)
    _G.EspHealthEnabled = state
    if not state then
        for player in pairs(healthCache) do
            removeHealthBar(player)
        end
    else
        for _, player in ipairs(Players:GetPlayers()) do
